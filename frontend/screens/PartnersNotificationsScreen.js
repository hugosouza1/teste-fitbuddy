...
import RequestsList from '../components/PartnersNotification/RequestsList'
...
const PartnersNotifications = () => {
  ...
  const fetchNotifications = async () => {
    ...
    if(!userEmail) throw new Error("Usuário não encontrado")
      ...
      let url = `${BACKEND_URL}/api/notifications/friendship/${userEmail}`
      if(solicitacoes.length > 0) {
        let elemento = solicitacoes[solicitacoes.length - 1]
        url += `?from=${elemento.data.toISOString()}`
      }
    ...
    try {
      ...
      setSolicitacoes((prev) => prev.concat(data))
    } catch(err) {
      console.log(err)
      return false
    }
...
export default PartnersNotifications